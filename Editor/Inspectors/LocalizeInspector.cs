	...
	public partial class LocalizeInspector : Editor
	{
		...
		bool OnGUI_SelectKey( ref string Term, bool Inherited )  // Inherited==true means that the mTerm is empty and we are using the Label.text instead
		{
			...
			if (mAllowEditKeyName)
			{
				...
				GUI.changed = false;
				mNewKeyName = EditorGUILayout.TextField(mNewKeyName, new GUIStyle("ToolbarSearchTextField"), GUILayout.ExpandWidth(true));
				if (GUI.changed)
				{
				...
				if (GUILayout.Button (string.Empty, string.IsNullOrEmpty(mNewKeyName) ? "ToolbarSearchCancelButtonEmpty" : "ToolbarSearchCancelButton", GUILayout.ExpandWidth(false)))
				{
					mTermsArray = null;	// regenerate this array to apply filtering
